...
export async function composeDjCommentary({
  currentSong,
  previousSong = null,
  upcomingSong = null,
  trigger = '切歌',
  ...
  const upcomingLine = upcomingSong?.name ? formatSong(upcomingSong) : '无';
  ...
  const isTransitionPreview = Boolean(upcomingSong?.name) && /尾段|预告|过渡|切换|结尾/.test(String(trigger || ''));
  ...
  const system = [
    ...
    isTransitionPreview
      ? '你的任务是在上一首尾段说出自然的过渡预告，让听众顺着余味进入下一首。'
      : '你的任务是在每首歌开始时说一段自然的开场介绍。',
    '要求：',
    '- 只输出最终文案，不要 JSON，不要引号，不要列表，不要解释。',
    isTransitionPreview
      ? '- 中文输出，控制在 45 到 110 个字，一到两句，口语一点，像真正播台的人。'
      : '- 中文输出，控制在 60 到 150 个字，两到三句。',
    '- 语气要自然，别像说明书，也别像客服回执。',
    isTransitionPreview
      ? '- 必须先把上一首尾声接住，再顺势引到下一首，不要等下一首已经开始才说。'
    ...
    '- 口吻要像成熟但自然的中文电台主持人：温和、有信息量、会聊天，不鸡汤，不堆形容词。',
  ].join('\n');
